...
#creates the diode model
def diode_model(V, I0, eta):

    return I0*(np.exp(scp.elementary_charge*V/(eta*scp.Boltzmann*T)) - 1)

popt_A, pcov_A = curve_fit(
    diode_model,
    A_V,
    A_I,
    p0=[1e-12, 1.5],        # initial guesses: I0 and eta
    bounds=([1e-20, 0.5],   # lower bounds
            [1e-3, 5])      # upper bounds
)

I0_fit_A, eta_fit_A = popt_A
